...
int main(){
    ...
    for(i = 0; i < 200; i++){
        ...
        preState.hand[player][handpos] = smithy;
        ...
        myAssert(preState.handCount[player] +2 == testState.handCount[player], "Net gain of +2 cards", &cardsDrawn, &cardsDrawnTotal);
        
        //insures cards come from the players hand
        myAssert(preState.deckCount[player] -3 ==  testState.deckCount[player], "Cards came from players hand", &sameDeck, &sameDeckTotal);
        
        // insure a card was played/discarded
        myAssert(preState.playedCardCount +1 == testState.playedCardCount, "smithy was discarded", &cardCount, &cardCountTotal);

    	//insure the smithy card was played
    	myAssert(testState.playedCards[testState.playedCardCount - 1] == smithy, "council_room card was played", &cardPlayed, &cardPlayedTotal);
	...
	printf("Hand count increased by 2 passed %d of %d times\n", cardsDrawn, cardsDrawnTotal);
	...
	printf("smithy card was played passed %d of %d times\n", cardPlayed, cardPlayedTotal);
